############################################################
# Shrek 2 and 3 Dialogue Project – Exploratory Data Analysis (EDA)
#
# Description:
#   Analysis of character dialogues in Shrek 2 and 3, exploring
#   speaking patterns, line distribution, and trends.
#
# Authors:
#   - Alicja Żebiałowicz
#   - Bruno Podbielski
...
# To save this plot, uncomment the code below:
...
srt_shrek <- read_srt("data/Shrek.2001.1080p.BluRay.x264.Ganool.srt")
csv_shrek2 <- read.csv("data/shrek2.csv")
srt_shrek2 <- read_srt("data/Shrek2.srt")
csv_shrek3 <- read.csv("data/shrek3.csv") 
...
fiona <- circle_crop("data/fiona.png", border_size = 4)
kot <- circle_crop("data/kot.png", border_size = 4)
krol <- circle_crop("data/krol.png", border_size = 4)
ksiaze <- circle_crop("data/ksiaze.png", border_size = 4)
osiol <- circle_crop("data/osiol.png", border_size = 4)
shrek <- circle_crop("data/shrek.png", border_size = 4)
wrozka <- circle_crop("data/wrozka.png", border_size = 4)
krolowa <- circle_crop("data/krolowa.png", border_size = 4)
pinokio <- circle_crop("data/pinokio.png", border_size = 4)
ciastek <- circle_crop("data/ciastek.png", border_size = 4)
...
# ggsave(
#   plot = p,
#   filename = "slupkowy_ze_zdjeciami.png",
#   bg = "transparent"
# )
...
shrek2 <- read.csv("data/shrek2.csv")
shrek2 <- shrek2 %>% 
...
fiona <- readPNG(circle_crop("data/fiona.png", border_size = 4))
kot <- readPNG(circle_crop("data/kot.png", border_size = 4))
krol <- readPNG(circle_crop("data/krol.png", border_size = 4))
ksiaze <- readPNG(circle_crop("data/ksiaze.png", border_size = 4))
osiol <- readPNG(circle_crop("data/osiol.png", border_size = 4))
shrek <- readPNG(circle_crop("data/shrek.png", border_size = 4))
wrozka <- readPNG(circle_crop("data/wrozka.png", border_size = 4))
krolowa <- readPNG(circle_crop("data/krolowa.png", border_size = 4))
pinokio <- readPNG(circle_crop("data/pinokio.png", border_size = 4))
ciastek <- readPNG(circle_crop("data/ciastek.png", border_size = 4))
...
shrek3 <- na.omit(csv_shrek3) %>%
  ...
  mutate(scene = case_when(timestamp <= 197.1230 ~ "Scene01",
                           ...
                           1429.8225 < timestamp & timestamp <= 1493.7660 ~ "Scene09",
...
# Lists of 2- and 3-Element Character Names
...
# Concatenating lines based on speaker:
#
# Lines are merged when the speaker in the previous line
# is the same as in the current line.
# Lines with dialogues from more than one character are ignored.
